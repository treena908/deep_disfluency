...
class DeepDisfluencyTagger(IncrementalTagger):
    ...
    def __init__(self, config_file=None,
        ...
        if self.args.model_type=='keras_LSTM':
            self.model = self.init_model_from_config(self.args,saved_model_dir)
        else:
            self.model = self.init_model_from_config(self.args, None)
    ...
    def init_model_from_config(self, args,folder):
        ...
        lr = args.lr
        print ("Initializing model of type"+ str(self.model_type)+ "...")
        ...
        elif self.model_type == 'keras_LSTM':
            emb=self.load_embeddings(self.args.embeddings)
            model = keras_LSTM(sequence_len=256,emb=emb,folder=folder,ne=vocab_size,
    ...
    def evaluate_fast_from_matrices(self, validation_matrices, tag_file,
        ...
        for idx,item in enumerate(idx_to_label_dict.items()):
            ...
            # if (type(idx_to_label_dict[idx])==tuple):

            log_file.write('{}{}{}{}'.format(idx, "  ", item, type(item)))
            log_file.write('\n')
        ...
        log_file.close()
        return results
    def make_tag_to_index_map(self):
        log_file=open('tag_2_idx.txt','w')
        tag_2_idx_map={}
        for key,value in self.tag_to_index_map.items():
            # log_file.write(str(key)+" "+str(value)+" "+(str(type(key))+" "+str(type(value))+"\n"))
            if type(key)==str and key!=0:
                # log_file.write('inside func \n')
                # log_file.write(str(key) + " " + str(value) + " " + (str(type(key)) + " " + str(type(value)) + "\n"))
                tag_2_idx_map[value]=key
        log_file.write("final \n"+str(tag_2_idx_map))
        log_file.close()
        return tag_2_idx_map
    def evaluate_result_from_trained_model(self,
        ...
        # print(self.tag_to_index_map.items())
        log_file.write('dict check')
        log_file.write('\n')
        log_file.write(str(self.tag_to_index_map.items()))
        log_file.write('\n')
        idx_2_label_dict =self.make_tag_to_index_map()
        # idx_2_label_dict = {v: k for k, v in self.tag_to_index_map.items()}
        log_file.write('idx_2_label_dict')
        log_file.write('\n')
        log_file.write(str(idx_2_label_dict))
        log_file.write('\n')
    ...
    def train_net(self, train_dialogues_filepath=None,
        ...
        for e in range(start, self.args.n_epochs + 1):
            ...
            if self.args.decay and (e - best_epoch) > 1:
                ...
                decay_rate = 0.85
                lr *= decay_rate
                print ("learning rate decayed, now %.6f" %lr)
            if lr < 1e-5:
